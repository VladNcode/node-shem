...
const routing = {
	user: {
		...
		async post({ login, password }) {
			const sql = 'INSERT INTO users (login, password) VALUES ($1, $2) RETURNING id, login';
			const passwordHash = await hash(password);
			return pool.query(sql, [login, passwordHash]);
		},

		async put(id, { login, password }) {
			const sql = 'UPDATE users SET login = $1, password = $2 WHERE id = $3 RETURNING id, login';
			const passwordHash = await hash(password);
			return pool.query(sql, [login, passwordHash, id]);
		},

		delete(id) {
			const sql = 'DELETE FROM users WHERE id = $1 RETURNING id, login';
			return pool.query(sql, [id]);
		},
	},
};
